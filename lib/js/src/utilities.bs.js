...
function getOwnPropertySymbols(obj) {
  ...
  if (typeof match === "number") {
    ...
    return Object.getOwnPropertySymbols(match[0]);
  } else {
    return /* array */[];
  }
}
...
function ofReasonAction(action) {
  var symbols = getOwnPropertySymbols(action);
...
function symbolValue(action, debugSymbol) {
  return Belt_Option.map(Belt_Array.get(Belt_Array.keep(ofReasonAction(action), (function (param) {
                ...
                var symbol = Caml_array.caml_array_get(getOwnPropertySymbols(action), param[0]);
